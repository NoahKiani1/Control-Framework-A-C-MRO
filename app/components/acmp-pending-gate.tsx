...
type AcmpPendingGateProps = {
  suppress?: boolean;
};

export function AcmpPendingGate({ suppress = false }: AcmpPendingGateProps) {
  ...
  useEffect(() => {
    if (role !== "office" || hiddenForPath || suppress) {
      return;
    }
  ...
  if (hiddenForPath || role !== "office" || suppress || summary.total <= 0) {
    return null;
  }
